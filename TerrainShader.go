...
const(
	TERRAIN_VERTEX_FILE = `#version 450 core
...
void main(void) {
	...
	TERRAIN_FRAGMENT_FILE = `#version 450 core
...
func newTerrainShader() TerrainShader {
	return TerrainShader{newShaderProgram(TERRAIN_VERTEX_FILE, TERRAIN_FRAGMENT_FILE, bindTerrainAttributes, getAllTerrainUniformLocations)}
}

func (s TerrainShader) loadTransformationMatrix(matrix mgl32.Mat4) {
	s.loadMatrix(s.uniformLocations["transformationMatrix"], matrix)
}

func (s TerrainShader) loadProjectionMatrix(matrix mgl32.Mat4) {
	s.loadMatrix(s.uniformLocations["projectionMatrix"], matrix)
}

func (s TerrainShader) loadViewMatrix(camera Camera) {
	viewMatrix := createViewMatrix(camera)
	s.loadMatrix(s.uniformLocations["viewMatrix"], viewMatrix)
}

func (s TerrainShader) loadLight(light Light) {
	s.loadVector(s.uniformLocations["lightPosition"], light.position)
	s.loadVector(s.uniformLocations["lightColour"], light.colour)
}

func (s TerrainShader) loadShineVariables(damper, reflectivity float32) {
	s.loadFloat(s.uniformLocations["shineDamper"], damper)
	s.loadFloat(s.uniformLocations["reflectivity"], reflectivity)
}

func (s TerrainShader) loadSkyColour(r,g,b float32) {
	s.loadVector(s.uniformLocations["skyColour"], mgl32.Vec3{r,g,b})
}


func bindTerrainAttributes(program ShaderProgram) {
...
func getAllTerrainUniformLocations(program ShaderProgram) {
